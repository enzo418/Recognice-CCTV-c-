...
namespace json_dto {
    ...
    inline void validator_error(const std::string& error_message) {
       ...
       public:
        ...
        void operator()(Number value) const {
            if (m_min_value > value || m_max_value < value)
                validator_error("invalid value: " + std::to_string(value) +
                                ", must be in "
                                "[ " +
        ...
        void operator()(const std::vector<Number>& values) const {
            for (auto v : values) (*this)(v);
        }

        template <typename Field_Inner_Type>
        void operator()(const nullable_t<Field_Inner_Type>& value) const {
            if (value) (*this)(*value);
        }
    ...
    class one_of_validator_t {
       public:
        ...
        void operator()(const Field_Type& value) const {
            if (m_values.cend() ==
                std::find(m_values.cbegin(), m_values.cend(), value)) {
                validator_error(
                    "invalid value, must be one of predefined values");
            }
        }

        void operator()(const std::vector<Field_Type>& values) const {
            for (auto v : values) (*this)(v);
        }
