...
namespace json_dto {
    ...
    namespace details {
        ...
        namespace meta {
            ...
            template <typename T>
            struct has_key_type<T, void_t<typename T::key_type> >
                : public std::true_type {};
            ...
            template <typename T>
            struct has_iterator_type<T, void_t<typename T::iterator> >
                : public std::true_type {};
            ...
            template <typename T>
            struct has_const_iterator_type<T,
                                           void_t<typename T::const_iterator> >
                : public std::true_type {};
            ...
            template <typename T>
            struct has_begin<
                T, void_t<decltype(std::declval<const T&>().begin())> >
                : public std::true_type {};
            ...
            template <typename T>
            struct has_end<T, void_t<decltype(std::declval<const T&>().end())> >
                : public std::true_type {};
            ...
            struct has_emplace_back<
                T, void_t<decltype(std::declval<T&>().emplace_back(
                       std::declval<typename T::value_type>()))> >
                : public std::true_type {};
            ...
            struct has_emplace_after<
                T, void_t<decltype(std::declval<T&>().emplace_after(
            ...
            struct is_stl_like_sequence_container {
                static constexpr bool value =
                    has_value_type<T>::value && !has_key_type<T>::value &&
                    has_iterator_type<T>::value &&
                    has_const_iterator_type<T>::value && has_begin<T>::value &&
                    has_end<T>::value &&
                    (has_emplace_back<T>::value ||
                     (has_before_begin<T>::value &&
                      has_emplace_after<T>::value));
            };
            ...
            struct is_stl_like_associative_container {
                static constexpr bool value =
                    ...
                    has_iterator_type<T>::value &&
                    has_const_iterator_type<T>::value && has_begin<T>::value &&
                    has_end<T>::value && has_emplace<T>::value;
            };
        ...
        namespace sequence_containers {
            ...
            template <typename C>
            using container_filler_t =
                container_filler_impl_t<C,
                                        meta::has_before_begin<C>::value &&
    ...
    class ex_t : public std::runtime_error {
       ...
       public:
        ex_t(const std::string& error_desc) : base_type_t {error_desc} {}
    };
    ...
    class json_input_t {
       public:
        json_input_t(const rapidjson::Value& object) : m_object {object} {}

        template <typename Binder>
        json_input_t& operator&(const Binder& b) {
    ...
    class json_output_t {
       public:
        json_output_t(rapidjson::Value& object,
        ...
        template <typename Binder>
        json_output_t& operator&(const Binder& b) {
     ...
     */
    template <typename T>
    const_map_key_t<T> const_map_key(const T& v) noexcept {
        return {v};
    };
     ...
     *
     * Since v.0.2.11 json_dto makes distinction between keys and values
     * in map-like containers. Values are deserialized by calling
     * read_json_value for a reference of type T (where T is a type of a value).
     * But keys are now deserialized by calling read_json_value for an instance
     * of type mutable_map_key_t<T> (where T is a type of a key).
     *
     ...
     */
    template <typename T>
    mutable_map_key_t<T> mutable_map_key(T& v) noexcept {
        return {v};
    };
    ...
    struct default_reader_writer_t {
        template <typename Field_Type>
        void read(Field_Type& v, const rapidjson::Value& from) const;

        template <typename Field_Type>
        void write(const Field_Type& v, rapidjson::Value& to,
...
#define RW_JSON_VALUES(type, checker, getter, setter)                      \
    inline void read_json_value(type& v, const rapidjson::Value& object) { \
        if (object.checker())                                              \
            v = object.getter();                                           \
        else                                                               \
            throw ex_t {"value is not " #type};                            \
    }                                                                      \
    inline void write_json_value(type v, rapidjson::Value& object,         \
    ...
    inline void read_json_value(std::uint16_t& v,
    ...
    inline void write_json_value(std::uint16_t v, rapidjson::Value& object,
                                 rapidjson::MemoryPoolAllocator<>& allocator) {
    ...
    inline void read_json_value(std::int16_t& v,
                                const rapidjson::Value& object) {
        std::int32_t value;
        read_json_value(value, object);
    ...
    inline void write_json_value(std::int16_t v, rapidjson::Value& object,
                                 rapidjson::MemoryPoolAllocator<>& allocator) {
    ...
    inline void read_json_value(std::uint8_t& v,
                                const rapidjson::Value& object) {
        std::uint32_t value;
        read_json_value(value, object);
    ...
    inline void write_json_value(std::uint8_t v, rapidjson::Value& object,
                                 rapidjson::MemoryPoolAllocator<>& allocator) {
        const std::uint32_t value = v;
        write_json_value(value, object, allocator);
    }
    ...
    inline void read_json_value(std::int8_t& v,
                                const rapidjson::Value& object) {
        std::int32_t value;
        read_json_value(value, object);
    ...
    inline void write_json_value(std::int8_t v, rapidjson::Value& object,
                                 rapidjson::MemoryPoolAllocator<>& allocator) {
        const std::int32_t value = v;
        write_json_value(value, object, allocator);
    }
    ...
    inline void read_json_value(std::string& s,
                                const rapidjson::Value& object) {
        if (object.IsString())
            s = object.GetString();
        else
    ...
    inline void write_json_value(const std::string& s, rapidjson::Value& object,
                                 rapidjson::MemoryPoolAllocator<>& allocator) {
        constexpr std::string::size_type max_str_len =
            std::numeric_limits<rapidjson::SizeType>::max();

        if (max_str_len < s.size()) {
            throw ex_t {
                "string length is too large: " + std::to_string(s.size()) +
                " (max is " + std::to_string(max_str_len) + ")"};
        }

        object.SetString(s.data(), static_cast<rapidjson::SizeType>(s.size()),
                         allocator);
    }

    // Since v.0.2.10.
    inline void write_json_value(const rapidjson::Value::StringRefType& s,
    ...
    // Since v.0.2.11.
    template <typename T>
    void read_json_value(mutable_map_key_t<T> key,
                         const rapidjson::Value& object) {
    ...
    // Since v.0.2.11.
    template <typename T>
    void write_json_value(const_map_key_t<T> key, rapidjson::Value& object,
                          rapidjson::MemoryPoolAllocator<>& allocator) {
    ...
    inline void read_json_value(rapidjson::Document& d,
                                const rapidjson::Value& object) {
        d.CopyFrom(object, d.GetAllocator());
    }

    inline void write_json_value(const rapidjson::Document& d,
                                 rapidjson::Value& object,
                                 rapidjson::MemoryPoolAllocator<>& allocator) {
    ...
    inline void read_json_value(cpp17::optional<T>& v,
                                const rapidjson::Value& object,
                                Reader_Writer reader_writer = Reader_Writer {});

    template <typename T, typename Reader_Writer = default_reader_writer_t>
    inline void write_json_value(
        const cpp17::optional<T>& v, rapidjson::Value& object,
        rapidjson::MemoryPoolAllocator<>& allocator,
        Reader_Writer reader_writer = Reader_Writer {});
#endif
    ...
    void read_json_value(std::vector<T, A>& vec, const rapidjson::Value& object,
                         const Reader_Writer& reader_writer = Reader_Writer {});

    template <typename T, typename A,
              typename Reader_Writer = default_reader_writer_t>
    void write_json_value(
        const std::vector<T, A>& vec, rapidjson::Value& object,
        rapidjson::MemoryPoolAllocator<>& allocator,
        const Reader_Writer& reader_writer = Reader_Writer {});
    ...
    std::enable_if_t<details::meta::is_stl_like_sequence_container<C>::value,
                     void>
    read_json_value(C& cnt, const rapidjson::Value& object,
                    const Reader_Writer& reader_writer = Reader_Writer {});

    template <typename C, typename Reader_Writer = default_reader_writer_t>
    std::enable_if_t<details::meta::is_stl_like_sequence_container<C>::value,
                     void>
    write_json_value(const C& cnt, rapidjson::Value& object,
                     rapidjson::MemoryPoolAllocator<>& allocator,
                     const Reader_Writer& reader_writer = Reader_Writer {});
    ...
    template <typename C, typename Reader_Writer = default_reader_writer_t>
    std::enable_if_t<
        details::meta::is_stl_set_like_associative_container<C>::value, void>
    read_json_value(C& cnt, const rapidjson::Value& object,
                    const Reader_Writer& reader_writer = Reader_Writer {});
    ...
    template <typename C, typename Reader_Writer = default_reader_writer_t>
    std::enable_if_t<
        details::meta::is_stl_map_like_associative_container<C>::value, void>
    read_json_value(
        C& cnt, const rapidjson::Value& object,
    ...
    template <typename Field_Type>
    struct nullable_t {
        nullable_t() noexcept : m_has_value {false} {}

        nullable_t(std::nullptr_t) noexcept : m_has_value {false} {}

        explicit nullable_t(Field_Type value) : m_has_value {true} {
            new (m_image_space) Field_Type {std::move(value)};
        }

        nullable_t(const nullable_t& other) : m_has_value {other.m_has_value} {
            if (has_value()) new (m_image_space) Field_Type {other.field_ref()};
        }

        nullable_t(nullable_t&& other) : m_has_value {other.m_has_value} {
            if (has_value())
                new (m_image_space) Field_Type {std::move(other.field_ref())};
        }

        template <typename... Args>
        explicit nullable_t(Args&&... args) : m_has_value {true} {
        ...
        nullable_t& operator=(const nullable_t& other) {
        ...
        nullable_t& operator=(nullable_t&& other) {
        ...
        nullable_t& operator=(const Field_Type& value) {
        ...
        nullable_t& operator=(Field_Type&& value) {
        ...
        nullable_t& operator=(std::nullptr_t) noexcept {
            reset();
            return *this;
        }

        const Field_Type* operator->() const noexcept { return field_ptr(); }

        Field_Type* operator->() noexcept { return field_ptr(); }

        const Field_Type& operator*() const noexcept { return field_ref(); }

        Field_Type& operator*() noexcept { return field_ref(); }

        bool operator==(const nullable_t& other) const {
            return has_value() && other.has_value() &&
                   field_ref() == other.field_ref();
        }

        void emplace() { *this = Field_Type {}; }

        void emplace(Field_Type value) { *this = std::move(value); }
       ...
       private:
        ...
        Field_Type* field_ptr() noexcept {
            return reinterpret_cast<Field_Type*>(m_image_space);
        }

        const Field_Type* field_ptr() const noexcept {
            return reinterpret_cast<const Field_Type*>(m_image_space);
        }

        Field_Type& field_ref() noexcept { return *field_ptr(); }

        const Field_Type& field_ref() const noexcept { return *field_ptr(); }
    };
    ...
    */
    ...
    std::enable_if_t<!details::meta::is_stl_like_container<Dto>::value, void>
    ...
    template <typename Dto>
    std::enable_if_t<!details::meta::is_stl_like_container<Dto>::value, void>
    write_json_value(const Dto& v, rapidjson::Value& object,
                     rapidjson::MemoryPoolAllocator<>& allocator) {
    ...
    template <typename Field_Type,
              typename Reader_Writer = default_reader_writer_t>
    void read_json_value(nullable_t<Field_Type>& f,
                         const rapidjson::Value& object,
                         Reader_Writer reader_writer = Reader_Writer {}) {
        Field_Type value;
    ...
    template <typename Field_Type,
              typename Reader_Writer = default_reader_writer_t>
    void write_json_value(const nullable_t<Field_Type>& f,
                          rapidjson::Value& object,
                          rapidjson::MemoryPoolAllocator<>& allocator,
                          Reader_Writer reader_writer = Reader_Writer {}) {
        if (f)
    ...
    template <typename T, typename Reader_Writer>
    inline void write_json_value(const cpp17::optional<T>& v,
                                 rapidjson::Value& object,
                                 rapidjson::MemoryPoolAllocator<>& allocator,
                                 Reader_Writer reader_writer) {
        if (v) reader_writer.write(*v, object, allocator);
    }
#endif
    ...
    template <typename T, typename A, typename Reader_Writer>
    void write_json_value(const std::vector<T, A>& vec,
                          rapidjson::Value& object,
                          rapidjson::MemoryPoolAllocator<>& allocator,
                          const Reader_Writer& reader_writer) {
        object.SetArray();
        for (typename details::std_vector_item_read_access_type<T>::type v :
             vec) {
            rapidjson::Value o;
    ...
    read_json_value(C& cnt, const rapidjson::Value& object,
        ...
        if (!object.IsArray())
            throw ex_t {
                "value can't be deserialized into std::set-like container!"};
    ...
    write_json_value(const C& cnt, rapidjson::Value& object,
                     rapidjson::MemoryPoolAllocator<>& allocator,
                     const Reader_Writer& reader_writer) {
        const auto write_item = [&object, &allocator,
                                 &reader_writer](const auto& v) {
            rapidjson::Value o;
            reader_writer.write(v, o, allocator);
            object.PushBack(o, allocator);
        };
        ...
        object.SetArray();
        for (const auto& v : cnt) write_item(v);
    }
    ...
    read_json_value(C& cnt, const rapidjson::Value& object,
        ...
        if (!object.IsObject())
            throw ex_t {
                "value can't be deserialized into std::map-like container!"};
        ...
        for (auto it = object.MemberBegin(); it != object.MemberEnd(); ++it) {
            ...
            // a non-const reference to it to read() method of the
            // reader_writer.
            ...
            cnt.emplace(
                typename C::value_type {std::move(key), std::move(value)});
        }
    }
    ...
    write_json_value(const C& cnt, rapidjson::Value& object,
        ...
        object.SetObject();
        for (const auto& kv : cnt) write_item(kv);
    }
    ...
    namespace details {
        ...
        struct stl_like_container_reader_t {
            ...
            void read_from(const rapidjson::Value& from) const {
        ...
        struct stl_like_container_writer_t {
            ...
            void write_to(rapidjson::Value& to,
                          rapidjson::MemoryPoolAllocator<>& allocator) const {
    ...
    template <typename C>
    std::enable_if_t<details::meta::is_stl_like_container<C>::value, void>
     ...
     *
     * Throws an exception because non-nullable attribute can't receive null
     * value.
     *
     * Is has name `default_on_null` since v.0.3.0.
     */
    template <typename Field_Type>
    void default_on_null(Field_Type&) {
     ...
     */
    template <typename Field_Type>
    void default_on_null(nullable_t<Field_Type>& f) {
        f.reset();
    }

    template <typename Field_Type, typename Field_Default_Value_Type>
    void set_default_value(Field_Type& f,
                           Field_Default_Value_Type&& default_value) {
        f = std::move(default_value);
    }

    template <typename Field_Type, typename Field_Default_Value_Type>
    void set_default_value(nullable_t<Field_Type>& f,
                           Field_Default_Value_Type&& default_value) {
    ...
    struct mandatory_attr_t {
        ...
        template <typename Field_Type>
        constexpr bool is_default_value(Field_Type&) const noexcept {
            return false;
        }
    };
    ...
    struct optional_attr_t {
        ...
        template <typename Field_Type>
        void on_field_not_defined(Field_Type& f) const {
        ...
        template <typename Field_Type>
        bool is_default_value(nullable_t<Field_Type>& f) const {
            return f && *f == m_default_value;
        }

        template <typename Field_Type>
        bool is_default_value(Field_Type& f) const {
    ...
    struct optional_attr_null_t {
        template <typename Field_Type>
        void on_field_not_defined(nullable_t<Field_Type>& f) const noexcept {
            f.reset();
        }
        ...
        template <typename Field_Type>
        bool is_default_value(nullable_t<Field_Type>& f) const noexcept {
            return !f.has_value();
        }
    };
    ...
    //! Field set/notset attribute ckecker for optional case without default
    //! value.
    struct optional_nodefault_attr_t {
        template <typename Field_Type>
        constexpr void on_field_not_defined(Field_Type&) const noexcept {}
     ...
     * @since v.0.2.9
     */
    inline string_ref_t make_string_ref(const char* src,
     ...
     * @since v.0.2.9
     */
    inline string_ref_t make_string_ref(const std::string& src) noexcept {
     ...
     * @since v.0.2.9
     */
    inline string_ref_t make_string_ref(const char* src) noexcept {
    ...
    struct empty_validator_t {
        template <typename Field_Type>
        constexpr void operator()(const Field_Type&) const noexcept {}
    };
    ...
    template <typename Field_Type>
    void default_reader_writer_t::read(Field_Type& v,
                                       const rapidjson::Value& from) const {
        read_json_value(v, from);
    }

    template <typename Field_Type>
    void default_reader_writer_t::write(
        const Field_Type& v, rapidjson::Value& to,
        rapidjson::MemoryPoolAllocator<>& allocator) const {
     ...
     * 		io & json_dto::mandatory(my_int_reader_writer{}, "field",
     * field_)
     * 			...
     * 			;
     * 	}
     ...
     * to integers.
     *
     * It seems that this problem can be solved by extending
     ...
     * 	template<typename Io> void json_io(Io & io) {
     * 		io & json_dto::mandatory(
     * 				// The first occurence of apply_to_content_t is for
     * nullable_t. json_dto::apply_to_content_t<
     * 					// The second occurence of apply_to_content_t is for
     * std::vector. json_dto::apply_to_content_t<
     * 						// This is for the content of
     * std::vector. my_int_reader_writer
     * 					>
     * 				>{},
     * 				"params", params_)
     * 			...
    ...
    struct apply_to_content_t {
        ...
        template <typename Field_Type>
        void write(Field_Type& v, rapidjson::Value& to,
                   rapidjson::MemoryPoolAllocator<>& allocator) const {
     ...
     * binder_t now just owns an instance of binder_data_holder_t. It allows a
    user
    ...
    // A special proxy for field that can only be serialized, but not
    deserialized. template< typename F > struct serialize_only_proxy_t
    {
    ...
    class binder_data_holder_t<
            ...
            :	public binder_data_holder_t<
                            Reader_Writer,
                            typename
    some_project::serialize_only_proxy_t<Field_Type>::field_type, Manopt_Policy,
                            Validator >
    {
    ...
    public:
            ...
            {} // The `proxy` parameter is no more needed and isn't used any
    more.
    };
    ...
    template <typename Reader_Writer, typename Field_Type,
              typename Manopt_Policy, typename Validator>
    class binder_data_holder_t {
       ...
       public:
        ...
        binder_data_holder_t(Reader_Writer&& reader_writer,
                             string_ref_t field_name, Field_Type& field,
                             Manopt_Policy&& manopt_policy,
                             Validator&& validator)
        ...
        const Reader_Writer& reader_writer() const noexcept {
            return m_reader_writer;
        }

        const string_ref_t& field_name() const noexcept { return m_field_name; }

        Field_Type& field_for_serialization() const noexcept { return m_field; }

        Field_Type& field_for_deserialization() const noexcept {
            return m_field;
        }

        const Manopt_Policy& manopt_policy() const noexcept {
            return m_manopt_policy;
        }

        const Validator& validator() const noexcept { return m_validator; }
    };
    ...
    // We have to specialize json_dto::binder_data_holder_t to store
    // the content of some_project::ignore_after_deserialization_proxy_t instead
    of
    ...
    class binder_data_holder_t<
                    Reader_Writer,
                    const
    some_project::ignore_after_deserialization_proxy_t<Field_Type>,
                    Manopt_Policy,
                    Validator >
    {
            ...
            ...
            void json_io(Json_Io & io)
            {
                    io
                            ...
                            & json_dto::mandatory("version",
                                            ...
                                            // NOTE: validators will be applied
    to deserialized value. json_dto::min_max_constraint(1,3)) & ...
                            ;
            }
    };
     * @endcode
    ...
    struct binder_read_from_implementation_t {
        static void read_from(const Binder_Data_Holder& binder_data,
                              const rapidjson::Value& object) {
            static_assert(
            ...
            if (!object.IsObject()) {
                throw ex_t {"unable to extract field \"" +
                            std::string {binder_data.field_name().s} +
                            "\": "
                            "parent json type must be object"};
            }
    ...
    struct binder_write_to_implementation_t<
                    binder_data_holder_t<
                            Reader_Writer,
                            const
    some_project::deserialize_only_proxy_t<Field_Type>, Manopt_Policy, Validator
                    >
            >
    {
            using data_holder_t = binder_data_holder_t<
            ...
            void json_io(Json_Io & io)
            {
                    io
                            ...
                            & json_dto::mandatory("priority",
                                            ...
                                            // NOTE: validators will be applied
    to deserialized value. json_dto::min_max_constraint(1,9)) & ...
                            ;
            }
    };
     * @endcode
    ...
    template <typename Binder_Data_Holder>
    struct binder_write_to_implementation_t {
        static void write_to(const Binder_Data_Holder& binder_data,
                             rapidjson::Value& object,
                             rapidjson::MemoryPoolAllocator<>& allocator) {
            binder_data.validator()(
            ...
            if (!binder_data.manopt_policy().is_default_value(
                ...
                binder_data.reader_writer().write(
                    binder_data.field_for_serialization(), value, allocator);

                object.AddMember(binder_data.field_name(), value, allocator);
            }
        }
    };
    ...
    /*!
     * @attention
     * Since v.0.2.12, additional care should be taken when working with
     * instances of binder_t. An instance of binder_t can hold a reference to a
     * temporary object. For example, in such a case:
     * @code
     * struct demo {
     ...
     * 		const auto keys_binder = json_dto::mandatory("keys", keys()); //
     * (1)
     * 		...
    ...
    template <typename Reader_Writer, typename Field_Type,
              typename Manopt_Policy, typename Validator>
    class binder_t {
        using data_holder_t = binder_data_holder_t<Reader_Writer, Field_Type,
                                                   Manopt_Policy, Validator>;

        using read_from_impl_t =
            binder_read_from_implementation_t<data_holder_t>;

        using write_to_impl_t = binder_write_to_implementation_t<data_holder_t>;

       public:
        binder_t(Reader_Writer&& reader_writer, string_ref_t field_name,
                 Field_Type& field, Manopt_Policy&& manopt_policy,
                 Validator&& validator)
            : m_data_holder {std::move(reader_writer), field_name, field,
                             std::move(manopt_policy), std::move(validator)} {}
        ...
        void read_from(const rapidjson::Value& object) const {
            try {
                ...
                throw ex_t {"error reading field \"" +
        ...
        void write_to(rapidjson::Value& object,
                      rapidjson::MemoryPoolAllocator<>& allocator) const {
            try {
                write_to_impl_t::write_to(m_data_holder, object, allocator);
            } catch (const std::exception& ex) {
                throw ex_t {"error writing field \"" +
                            std::string {m_data_holder.field_name().s} +
                            "\": " + ex.what()};
            }
        }
    ...
    template <typename Field_Type, typename Validator = empty_validator_t>
    auto mandatory(string_ref_t field_name, Field_Type&& field,
        ...
        using binder_type_t = binder_t<
            ...
            details::meta::field_type_from_reference_t<decltype(field)>,
            mandatory_attr_t, Validator>;

        return binder_type_t {default_reader_writer_t {}, field_name, field,
                              mandatory_attr_t {}, std::move(validator)};
    }

    //! Create bind for a mandatory JSON field with validator.
    template <typename Reader_Writer, typename Field_Type,
              typename Validator = empty_validator_t>
    auto mandatory(Reader_Writer reader_writer, string_ref_t field_name,
                   Field_Type&& field, Validator validator = Validator {}) {
        using binder_type_t = binder_t<
            Reader_Writer,
            // NOTE: since v.0.2.12 this way of detection of Field_Type
            // for binder_t must be used.
        ...
        return binder_type_t {std::move(reader_writer), field_name, field,
                              mandatory_attr_t {}, std::move(validator)};
    }
     ...
     * 	void json_io( Json_Io & io )
     * 	{
     * 		// headers_ receives std::vector<std::string>{} in case of
     * 'null'. io & json_dto::mandatory_with_null_as_default(
    ...
    auto mandatory_with_null_as_default(Reader_Writer reader_writer,
        ...
        using binder_type_t = json_dto::binder_t<
            ...
            // for binder_t must be used.
            details::meta::field_type_from_reference_t<decltype(field)>,
            mandatory_attr_with_null_as_default_t, Validator>;
     ...
     * 	{
     * 		// headers_ receives std::vector<std::string>{} in case of
     * 'null'. io & json_dto::mandatory_with_null_as_default( "headers",
     * headers_ )
     * 			...
     * 			;
     * 	}
    ...
    template <typename Field_Type,
              typename Validator = json_dto::empty_validator_t>
    auto mandatory_with_null_as_default(json_dto::string_ref_t field_name,
                                        Field_Type&& field,
                                        Validator validator = Validator {}) {
        return mandatory_with_null_as_default(
            default_reader_writer_t {}, field_name,
            std::forward<Field_Type>(field), std::move(validator));
    }
    ...
    template <typename Field_Type, typename Field_Default_Value_Type,
              typename Validator = empty_validator_t>
    auto optional(string_ref_t field_name, Field_Type&& field,
                  Field_Default_Value_Type default_value,
                  Validator validator = Validator {}) {
    ...
    template <typename Reader_Writer, typename Field_Type,
              ...
              typename Validator = empty_validator_t>
    auto optional(Reader_Writer reader_writer, string_ref_t field_name,
                  Field_Type&& field, Field_Default_Value_Type default_value,
                  Validator validator = Validator {}) {
        using opt_attr_t = optional_attr_t<Field_Default_Value_Type>;
        using binder_type_t = binder_t<
            Reader_Writer,
    ...
    template <typename Field_Type, typename Validator = empty_validator_t>
    auto optional_null(string_ref_t field_name, Field_Type&& field,
                       Validator validator = Validator {}) {
        using binder_type_t = binder_t<
            default_reader_writer_t,
            // NOTE: since v.0.2.12 this way of detection of Field_Type
            // for binder_t must be used.
            details::meta::field_type_from_reference_t<decltype(field)>,
            optional_attr_null_t, Validator>;

        return binder_type_t {default_reader_writer_t {}, field_name, field,
                              optional_attr_null_t {}, std::move(validator)};
    }

    //! Create bind for an optional JSON field with null default value .
    template <typename Reader_Writer, typename Field_Type,
              typename Validator = empty_validator_t>
    auto optional_null(Reader_Writer reader_writer, string_ref_t field_name,
                       Field_Type&& field, Validator validator = Validator {}) {
        using binder_type_t = binder_t<
            Reader_Writer,
            // NOTE: since v.0.2.12 this way of detection of Field_Type
    ...
    //! Create bind for an optional JSON field with null default value.
    template <typename Field_Type, typename Validator = empty_validator_t>
    auto optional(string_ref_t field_name, Field_Type& field, std::nullptr_t,
                  Validator validator = Validator {}) {
    ...
    template <typename Reader_Writer, typename Field_Type,
              typename Validator = empty_validator_t>
    auto optional(Reader_Writer reader_writer, string_ref_t field_name,
                  Field_Type& field, std::nullptr_t,
                  Validator validator = Validator {}) {
        return optional_null(std::move(reader_writer), field_name, field,
                             std::move(validator));
    }
    ...
    template <typename Field_Type, typename Validator = empty_validator_t>
    auto optional_no_default(string_ref_t field_name, Field_Type&& field,
                             Validator validator = Validator {}) {
        using binder_type_t = binder_t<
            default_reader_writer_t,
            // NOTE: since v.0.2.12 this way of detection of Field_Type
            // for binder_t must be used.
            details::meta::field_type_from_reference_t<decltype(field)>,
            optional_nodefault_attr_t, Validator>;
    ...
    template <typename Reader_Writer, typename Field_Type,
              typename Validator = empty_validator_t>
    auto optional_no_default(Reader_Writer reader_writer,
                             string_ref_t field_name, Field_Type&& field,
                             Validator validator = Validator {}) {
        using binder_type_t = binder_t<
            Reader_Writer,
    ...
    template <typename Dto>
    json_input_t& operator>>(json_input_t& i, Dto& v) {
        json_io(i, v);
        return i;
    }

    template <typename Dto>
    inline json_output_t& operator<<(json_output_t& o, const Dto& v) {
    ...
    template <typename Dto>
    std::string to_json(const Dto& dto) {
    ...
    struct pretty_writer_params_t {
        ...
        rapidjson::PrettyFormatOptions m_format_options {
            rapidjson::kFormatDefault};

        //
        // Setters
        //
        pretty_writer_params_t& indent_char(char ch) & {
            m_indent_char = ch;
            return *this;
        }
        ...
        pretty_writer_params_t& indent_char_count(unsigned count) & {
            if (!count) count = 1u;
        ...
        pretty_writer_params_t& format_options(
            rapidjson::PrettyFormatOptions opt) & {
    ...
    template <typename Dto>
    std::string to_json(const Dto& dto, pretty_writer_params_t writer_params) {
        rapidjson::Document output_doc;
        json_output_t jout {output_doc, output_doc.GetAllocator()};
        ...
        writer.SetIndent(writer_params.m_indent_char,
                         writer_params.m_indent_char_count);
        writer.SetFormatOptions(writer_params.m_format_options);
    ...
    inline void check_document_parse_status(
        const rapidjson::Document& document) {
        if (document.HasParseError()) {
            throw ex_t {std::string {"JSON parse error: '"} +
    ...
    Type from_json(const rapidjson::Value& json) {
    ...
    //! constructed DTO.
    template <typename Type>
    void from_json(const rapidjson::Value& json, Type& o) {
        json_input_t jin {json};
    ...
    template <typename Type,
              unsigned Rapidjson_Parseflags = rapidjson::kParseDefaultFlags>
    Type from_json(const std::string& json) {
    ...
    template <typename Type,
              unsigned Rapidjson_Parseflags = rapidjson::kParseDefaultFlags>
    void from_json(const std::string& json, Type& o) {
     ...
     */
    template <typename Type>
    void to_stream(std::ostream& to, const Type& type) {
        rapidjson::Document output_doc;
        json_dto::json_output_t jout {output_doc, output_doc.GetAllocator()};
    ...
    /*!
     * @brief Serialize an object into specified stream with using
     * pretty-writer.
     *
     * @since v.0.2.14
     */
    ...
    template <typename Type,
              unsigned Rapidjson_Parseflags = rapidjson::kParseDefaultFlags>
    void from_stream(std::istream& from, Type& o) {
    ...
    template <typename Type,
              unsigned Rapidjson_Parseflags = rapidjson::kParseDefaultFlags>
    Type from_stream(std::istream& from) {
        Type result;
