...
namespace {
    ...
    class cancelled_progressor : public net::progress_handler {
       public:
        ...
        float update(std::size_t dnow, std::size_t dtotal, std::size_t unow,
                     std::size_t utotal) override {
            (void)dnow;
            (void)dtotal;
            (void)unow;
            (void)utotal;
    ...
    netex::promise<net::content_t> download(std::string url) {
        return netex::make_promise<net::content_t>(
            [url = std::move(url)](auto resolve, auto reject) {
...
TEST_CASE("curly") {
    ...
    SUBCASE("wait") {
        {
            auto req =
                net::request_builder("https://httpbin.org/delay/1").send();
        ...
        {
            auto req =
                net::request_builder("https://httpbin.org/delay/2").send();
            REQUIRE(req.wait_for(net::time_sec_t(1)) ==
        ...
        }
        {
            auto req =
                net::request_builder("https://httpbin.org/delay/2").send();
            REQUIRE(req.wait_until(net::time_point_t::clock::now() +
                                   net::time_sec_t(1)) ==
                    net::req_status::pending);
            REQUIRE(req.wait_until(net::time_point_t::clock::now() +
                                   net::time_sec_t(5)) ==
    ...
    SUBCASE("cancel") {
        ...
        {
            auto req =
                net::request_builder("https://httpbin.org/status/200").send();
            REQUIRE(req.wait() == net::req_status::done);
    ...
    SUBCASE("is_done/is_pending") {
        ...
        {
            auto req = net::request_builder(net::http_method::POST,
    ...
    SUBCASE("get") {
        {
            auto req =
                net::request_builder("https://httpbin.org/status/204").send();
            auto resp = req.take();
            REQUIRE(req.status() == net::req_status::empty);
    ...
    SUBCASE("request_inspection") {
        {
            ...
            REQUIRE(content_j["headers"]["Custom-Header-1"] ==
                    "custom_header_value_1");
            REQUIRE(content_j["headers"]["Custom-Header-2"] ==
        ...
        }
        {
            auto resp =
                net::request_builder()
                    .url("https://httpbin.org/headers")
                    .headers({{"Custom-Header-1", "custom_header_value_1"},
    ...
    SUBCASE("response_inspection") {
        {
            auto req = net::request_builder()
                           .url(
                               "https://httpbin.org/"
                               "response-headers?hello=world&world=hello")
                           .method(net::http_method::GET)
                           .send();
        ...
        {
            std::map<std::string, std::string> qparams {{"hello", "world"},
                                                        {"world", "hello"}};
            auto req = net::request_builder()
                           .url("https://httpbin.org/response-headers")
                           .method(net::http_method::GET)
            ...
            REQUIRE(
                resp.last_url() ==
                "https://httpbin.org/response-headers?hello=world&world=hello");
            const auto content_j = json_parse(resp.content.as_string_view());
            REQUIRE(content_j["hello"] == "world");
            REQUIRE(content_j["world"] == "hello");
        }
    ...
    SUBCASE("binary") {
        {
            ...
            REQUIRE(resp.headers.at("Content-Type") ==
        ...
        }
        {
            auto resp = net::request_builder()
                            .url(
                                "http://httpbin.org/"
                                "drip?duration=2&numbytes=5&code=200&delay=1")
                            .method(net::http_method::GET)
                            .send()
                            .take();
            REQUIRE(resp.http_code() == 200u);
        ...
        }
        {
            auto req = net::request_builder()
                           .url(
                               "http://httpbin.org/"
                               "drip?duration=15&numbytes=5&code=200&delay=1")
                           .method(net::http_method::GET)
                           .response_timeout(net::time_sec_t(2))
                           .send();
            REQUIRE(req.wait_for(net::time_sec_t(1)) ==
                    net::req_status::pending);
            REQUIRE(req.wait_for(net::time_sec_t(5)) ==
                    net::req_status::timeout);
        }
        {
            auto resp =
                net::request_builder()
        ...
        }
        {
            ...
            REQUIRE(resp.headers.at("Content-Length") ==
                    std::to_string(untests::png_data_length));
        ...
        }
        {
            ...
            REQUIRE(!std::memcmp(std::move(resp.content).data().data(),
        ...
        }
        {
            ...
            REQUIRE(resp.headers.at("Content-Length") ==
                    std::to_string(untests::jpeg_data_length));
            REQUIRE_FALSE(resp.content.size());
        }
        {
            auto resp = net::request_builder()
            ...
            REQUIRE(!std::memcmp(std::as_const(resp.content).data().data(),
                                 untests::jpeg_data,
...
SUBCASE("callback") {
    {
        ...
        auto req =
            net::request_builder("http://www.httpbin.org/get")
    ...
    }
    {
        std::atomic_size_t call_once {0u};
        auto req =
            net::request_builder("http://www.httpbin.org/delay/2")
                .response_timeout(net::time_sec_t(0))
                .callback([&call_once](net::request request) {
                    std::this_thread::sleep_for(std::chrono::milliseconds(10));
                    ++call_once;
...
SUBCASE("callback_exception") {
    auto req =
        net::request_builder("http://www.httpbin.org/post")
...
TEST_CASE("curly/cancel_all_pending_requests") {
    SUBCASE("cancel new requests") {
        ...
        auto req1 =
            net::request_builder("https://httpbin.org/delay/2")
...
TEST_CASE("curly_examples") {
    ...
    SUBCASE("Get Requests") {
        ...
        std::cout << "Content type: " << response.headers["content-type"]
                  << std::endl;
        std::cout << "Body content: " << response.content.as_string_view()
                  << std::endl;
    ...
    SUBCASE("Post Requests") {
        ...
        std::cout << "Body content: " << response.content.as_string_view()
                  << std::endl;
        std::cout << "Content Length: " << response.headers["content-length"]
                  << std::endl;
    ...
    SUBCASE("Error Handling") {
        auto request =
            net::request_builder().url("http://unavailable.site.com").send();
